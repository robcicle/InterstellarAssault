...
namespace FX
{
	...
	void MyFX::SetupPointLight(int lightIdx, bool enable,
		...
		float range/*=1000.0f*/, float atten1/*=0.05f*/)
	{
		assert(lightIdx >= 0 && lightIdx < 8);
		if (!enable)
		{
	...
	bool MyFX::Init()
	{
		...
		pBuff = ReadAndAllocate("data/shaders/TextureVS.cso", bytes);
		...
		pBuff = ReadAndAllocate("data/shaders/PSLitNoTex.cso", bytes);
		CreatePixelShader(mD3D.GetDevice(), pBuff, bytes, mpPSLit);
		delete[] pBuff;

		pBuff = ReadAndAllocate("data/shaders/PSUnlitNoTex.cso", bytes);
		CreatePixelShader(mD3D.GetDevice(), pBuff, bytes, mpPSUnlit);
		delete[] pBuff;

		pBuff = ReadAndAllocate("data/shaders/PSLitTex.cso", bytes);
		CreatePixelShader(mD3D.GetDevice(), pBuff, bytes, mpPSLitTex);
		delete[] pBuff;

		pBuff = ReadAndAllocate("data/shaders/PSUnlitTex.cso", bytes);
